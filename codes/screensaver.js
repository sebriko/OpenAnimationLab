...
// Relaxing screensaver with wave-like movements
...
// Array for the floating circles
let circles = [];
let numCircles = 8;

// Create main circles - various sizes and colors
for (let i = 0; i < numCircles; i++) {
  ...
  // Arrange starting positions in a circle
...
// Additional small particles for depth effect
let particles = [];
let numParticles = 20;
...
for (let i = 0; i < numParticles; i++) {
  ...
  // Store starting position and index
  particle.startY = particle.y;
  particle.index = i;
  particles.push(particle);
}

// Central breathing circles for focus
...
let time = 0;
let breathPhase = 0;
let breathDuration = 4; // seconds per breath phase
...
mainTimer.onUpdate(function () {
  ...
  // Main circles in wave-like movement
  ...
  // Let particles float slowly
  for (let i = 0; i < particles.length; i++) {
    let particle = particles[i];
    particle.y -= 0.5;
    particle.x += Math.sin(time + particle.index) * 0.3;

    // Reset particle to bottom when it floats out at the top
    if (particle.y < -10) {
      particle.y = 730;
      particle.x = Math.random() * 1280;
    }
  ...
  // Gently animate decorative circles
  ...
  // Subtly change background color (various shades of blue)
...
// Second timer for slower effects
...
slowTimer.onUpdate(function () {
  ...
  // Slow rotation of decorative elements
