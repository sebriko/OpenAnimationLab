...
// PID Controller: Water Level Control
...
const GX = 560 - 10; // 10px to the left
const GY = 350 - 60 + 20 + 5; // 5px down
const GW = 690;
const GH = 330;
const GM_L = 55;
const GM_T = 10;
...
// ==================== SETPOINT LINE ====================
let setpointY = WB - (setpoint / 100) * WH;
let setpointLine = new Line(
  WL - 30,
  setpointY,
  WR + 30,
  setpointY,
...
setpointLine.setStrokeDash([8, 5]);

let setpointMarkerLabel = new Text("Setpoint", "Arial", FS, SOLL_COL);
setpointMarkerLabel.x = WR + 35;
setpointMarkerLabel.y = setpointY - 22;
...
// Setpoint and disturbance on the LEFT position (CX)
let setpointText = new Text(
  "Setpoint w = " + setpoint + " %",
  "Arial",
  FS,
  0x444444,
...
let setpointSlider = new ButtonSlider(10, 90, setpoint, 1, 50, 240);
setpointSlider.x = CX;
setpointSlider.y = CY + 28;

let distText = new Text(
  "Outflow (Disturbance) = " + disturbance + " %",
  "Arial",
  FS,
  0x444444,
);
...
// K-controllers on the RIGHT position (CX2)
let kpText = new Text(
  "K<sub>p</sub> = " + kp.toFixed(1),
  "Arial",
  FS,
  0x444444,
);
...
let xAxisLabel = new Text("Time", "Arial", FS, 0x444444);
xAxisLabel.x = PLOT_X + PLOT_W - 60;
xAxisLabel.y = PLOT_Y + PLOT_H + 3;

let yAxisLabel = new Text("Level", "Arial", FS, 0x444444);
yAxisLabel.x = PLOT_X - 80;
...
let graphSetpointLine = new LinePath(
  [[PLOT_X, PLOT_Y + PLOT_H / 2]],
  SOLL_COL,
  1.5,
);
graphSetpointLine.setAlpha(0.7);

let graphActualLine = new LinePath([[PLOT_X, PLOT_Y + PLOT_H]], IST_COL, 2.5);

let legendRect = new Rectangle(200, 26, 0xffffff);
legendRect.x = PLOT_X + PLOT_W - 205;
legendRect.y = GY + 3;
legendRect.setAlpha(0.9);
legendRect.setRoundedCorners(3);

let legSetpointLine = new Line(
  PLOT_X + PLOT_W - 198 - 25,
  GY + 16,
  PLOT_X + PLOT_W - 175 - 25,
  GY + 16,
  SOLL_COL,
  2,
);
legSetpointLine.setStrokeDash([5, 3]);
let legSetpointText = new Text("Setpoint", "Arial", 22, SOLL_COL);
legSetpointText.x = PLOT_X + PLOT_W - 178 - 25;
legSetpointText.y = GY - 6;

let legActualLine = new Line(
  PLOT_X + PLOT_W - 95,
  GY + 16,
  PLOT_X + PLOT_W - 72,
  GY + 16,
...
let legActualText = new Text("Actual", "Arial", 22, IST_COL);
legActualText.x = PLOT_X + PLOT_W - 75;
legActualText.y = GY - 6;
...
function updateVisuals() {
  ...
  setpointLine.setStart(WL - 30, sY);
  setpointLine.setEnd(WR + 30, sY);
  setpointMarkerLabel.x = WR + 35;
  setpointMarkerLabel.y = sY - 22;

  // Inflow: hide completely when controlOutput is practically 0
...
function updateSliderTexts() {
  setpointText.setText("Setpoint w = " + setpoint + " %");
  kpText.setText("K<sub>p</sub> = " + kp.toFixed(1));
  kiText.setText("K<sub>i</sub> = " + ki.toFixed(2));
  kdText.setText("K<sub>d</sub> = " + kd.toFixed(1));
  distText.setText("Outflow (Disturbance) = " + disturbance + " %");
}

function updateGraph() {
  if (histActual.length < 2) {
    graphActualLine.setPoints([[PLOT_X, PLOT_Y + PLOT_H]]);
    graphSetpointLine.setPoints([[PLOT_X, PLOT_Y + PLOT_H / 2]]);
    return;
  }

  let setpointPts = [];
  let actualPts = [];
  let n = histActual.length;

  for (let i = 0; i < n; i++) {
    let px = PLOT_X + (i / MAX_HIST) * PLOT_W;
    let actualPy = PLOT_Y + PLOT_H - (histActual[i] / 100) * PLOT_H;
    let setpointPy = PLOT_Y + PLOT_H - (histSetpoint[i] / 100) * PLOT_H;
    actualPts.push([px, actualPy]);
    setpointPts.push([px, setpointPy]);
  }

  graphActualLine.setPoints(actualPts);
  graphSetpointLine.setPoints(setpointPts);
}
...
timer.onUpdate(function () {
  ...
  ki = kiSlider.value;
  kd = kdSlider.value;
  setpoint = Math.round(setpointSlider.value);
  ...
  if (frameCountAnimation % 2 === 0) {
    histSetpoint.push(setpoint);
    histActual.push(waterLevel);

    if (histActual.length > MAX_HIST) {
      histSetpoint.shift();
      histActual.shift();
    }
