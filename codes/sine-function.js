...
let centerY = 360;
let circleX = 200;
let startX = circleX + amplitude + 50; // Curve starts to the right of the circle with distance
let endX = 1230;
let phase = 0;
...
// Initially set the helper line correctly
updateHelpLine();
...
wavelengthSlider.onChange((e) => {
  ...
  // Reset phase when wavelength is changed
  phase = 0;
  updateHelpLine();
});
...
amplitudeSlider.onChange((e) => {
  ...
  // Adjust circle and arrow
  myCircle.setRadius(amplitude);
  let arrowEndX = amplitude * Math.cos(phase);
  let arrowEndY = amplitude * Math.sin(phase);
  myArrow.setEnd(arrowEndX, arrowEndY);

  // Recalculate startX so curve stays to the right of the circle
...
function handleProgress() {
  ...
  // Calculate maximum phase (corresponds to the end of visible curve)
  let maxPhase = wavelength * (endX - startX);

  // Reset phase when end of curve is reached
  if (phase > maxPhase) {
    phase = 0;
  }
