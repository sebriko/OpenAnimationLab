...
// Comparison of spring-mass systems with and without damping
// ═══════════════════════════════════════════════════════════════════
...
const colors = {
  ...
  white: 0xffffff,
  grid: 0xcccccc,
  undamped: 0x0066cc, // Blue for undamped oscillation
  damped: 0xcc3300, // Red for damped oscillation
  spring: 0x666666, // Gray for the spring
  mass: 0x888888, // Mass
  envelope: 0xcc3300, // Envelope
};
...
let amplitude = 80; // Amplitude in pixels
let frequency = 0.5; // Frequency in Hz
let damping = 0.3; // Damping constant (only for damped)
let time = 0;
let dt = 1 / 60; // Time step
let isPlaying = false;

// Physical positions
let restPosition = 400; // Rest position of masses (y-coordinate)
let springTopY = 130; // Upper attachment point of spring
...
// Creates zigzag points for a spring
...
// Calculates the undamped displacement (starting at -A = upper position)
function getUndampedDisplacement(t) {
  let omega = 2 * Math.PI * frequency;
  return -amplitude * Math.cos(omega * t);
}

// Calculates the damped displacement (starting at -A = upper position)
...
// Calculates the dynamic delta based on frequency
function getDynamicDelta() {
  // Base delta at frequency 0.5 Hz is 3 pixels
  // At frequency 2.0 Hz it should be 3x smaller (i.e., 1 pixel)
  // Linear interpolation: delta = baseDelta / (1 + (f - 0.5) * 2/1.5)
  // Simplified: delta = baseDelta * 0.5 / max(frequency, 0.5)
  let baseDelta = 3;
  let referenceFreq = 0.5;

  if (frequency <= referenceFreq) {
    return baseDelta;
  }

  // At 2.0 Hz: delta = 3 * 0.5 / 2.0 = 0.75 ≈ 1 (i.e., about 3x smaller)
  return (baseDelta * referenceFreq) / frequency;
}

// ───────────────────────────────────────────────────────────────────
// SECTION 1: UNDAMPED OSCILLATION (left)
// ───────────────────────────────────────────────────────────────────
...
// Spring (undamped) - drawn as LinePath
...
// Curve for undamped oscillation
...
// Envelope for damped oscillation (upper)
...
// Markers for current position on the curves
...
// ───────────────────────────────────────────────────────────────────
// CONNECTION LINES: Mass to Curve
// ───────────────────────────────────────────────────────────────────
...
let legendText2 = new Text(
  "Damped: y(t) = -A·e<sup>-δt</sup>·cos(ωt)",
  "Arial",
  20,
  colors.damped,
  "left",
...
// ───────────────────────────────────────────────────────────────────
// CONTROLS: SLIDERS (all 3 side by side)
// ───────────────────────────────────────────────────────────────────

let sliderBaseX = 380; // Base position
let sliderSpacing = 310; // Spacing between sliders
let sliderY = 20; // Y position for all sliders
...
let labelFreq = new Text("Frequency f:", "Arial", 20, colors.dark, "left");
...
// Damping (third column)
let labelDamp = new Text("Damping δ:", "Arial", 20, colors.damped, "left");
...
let calcTitle = new Text("Current Values:", "Arial", 20, colors.dark, "left");
calcTitle.x = 40;
calcTitle.y = 550;
...
function updateMassPositions() {
  ...
  // Positions of masses (y is positive downward, displacement downward = positive)
  ...
  // Time span for the diagram
  let tMax = 10;
  let pixelsPerSecond = graphWidth / tMax;

  // Marker position on x-axis (moves right with time)
  let markerX = graphX + time * pixelsPerSecond;

  // Marker position on y-axis (same calculation as curve)
  // CORRECTED: Now with negative sign like the curves
  let omega = 2 * Math.PI * frequency;
  ...
  // Center of masses (for the connection line)
  let mass1CenterY = mass1Y + 25;
  let mass2CenterY = mass2Y + 25;

  // Horizontal connection lines (same Y coordinate)
  ...
  // Markers on the curves (move right)
...
function updateCurves() {
  ...
  // Time span: 10 seconds on the x-axis
  let tMax = 10;
  let pixelsPerSecond = graphWidth / tMax;

  // Dynamic delta based on frequency
  let deltaX = getDynamicDelta();

  for (let x = 0; x <= graphWidth; x += deltaX) {
    let t = x / pixelsPerSecond;

    // CORRECTED: Undamped curve with negative sign (minimum at t=0)
    let omega = 2 * Math.PI * frequency;
    let y1 = -amplitude * Math.cos(omega * t);
    pointsUndamped.push([x, y1]);

    // CORRECTED: Damped curve with negative sign (minimum at t=0)
    let decay = Math.exp(-damping * t);
  ...
  // Only show envelope when damping > 0
...
timer.onUpdate(function () {
  if (isPlaying) {
    ...
    // Stop animation when end of diagram is reached
    if (time >= tMax) {
      time = tMax;
      isPlaying = false;
      playButton.setText("Start");
