...
// Water Glass Simulation with Floating Ice Cubes
// Demonstration of Buoyancy and Wave Physics
...
// ==================== CONSTANTS ====================
const GLASS_X = 440;
const GLASS_Y = 150; // 50 pixels upward
const GLASS_WIDTH = 400;
const GLASS_HEIGHT = 500;
const GLASS_THICKNESS = 8;
const WATER_COLOR = 0x99d4ff; // Light blue
const ICE_COLOR = 0xd4f1f9;
const ICE_DENSITY = 0.917; // Ice floats ~92% submerged
const GRAVITY = 0.5; // Increased for faster falling
const WATER_DAMPING = 0.97;
const WAVE_SPEED = 0.12;
const SURFACE_POINTS = 50; // Number of points for the water surface
const ICE_MARGIN = 5; // Distance from glass wall
const MAX_ICE_CUBES = 12; // Maximum number of ice cubes

// Water level (height of water from bottom)
...
function createWaterPoints() {
  ...
  // Surface from left to right (with wave effect)
  for (let i = 0; i < SURFACE_POINTS; i++) {
...
let addIceButton = new Button("+ Ice Cube", 160, 40, "Arial", 22);
addIceButton.x = 200;
addIceButton.y = 20;

let countText = new Text("Ice Cubes: 0 / 12", "Arial", 22, 0x444444);
countText.x = 20;
countText.y = 70;
...
board.onMouseDown(function (e) {
  ...
  // Only in the area above the glass and above the water surface
  // With 5 pixel margin from the edges
  if (
    ...
    if (e.y < waterSurface - 10) {
      ...
      if (newCube) {
        newCube.y = e.y;
        countText.setText("Ice Cubes: " + iceCubes.length + " / 12");
      }
    }
  }
});
...
resetButton.onClick(function () {
  ...
  countText.setText("Ice Cubes: 0 / 12");
});
...
timer.onUpdate(function () {
  ...
  for (let cube of iceCubes) {
    ...
    // Wall collision – with 5 pixel margin
