...
// ── Inline BoardBase if not already loaded (for standalone / exported HTML) ──
if (typeof BoardBase === "undefined") {
  class _BoardBase {
    ...
    getAvailableSpace() {
      ...
      if (isFullscreen) {
        return { width: window.innerWidth, height: window.innerHeight, margin: 0 };
      }
      const previewRect = preview.getBoundingClientRect();
      const isMobileDevice = this.isMobile || window.innerWidth <= 768;
      let margin = 20;
      if (isMobileDevice) { margin = 5; }
      else if (previewRect.width < 400) { margin = 10; }
      else if (previewRect.width < 600) { margin = 15; }
      const availableWidth = Math.max(previewRect.width - margin * 2, 50);
      const availableHeight = Math.max(previewRect.height - margin * 2, 50);
      return {
        width: availableWidth, height: availableHeight, margin: margin,
        previewWidth: previewRect.width, previewHeight: previewRect.height,
    ...
    resizeCanvas() {
      ...
      this.resizeTimeout = setTimeout(() => { this.performResize(); }, 16);
    }

    prepareCanvasContainer() {
      if (!this.canvasContainer) { console.warn("Canvas container not found."); return; }
      ...
      if (preview) {
        ...
        const scale = Math.min(
          ...
          (previewRect.height - margin * 2) / this.height, 1,
        );
        ...
        Object.assign(this.canvasContainer.style, {
          ...
          left: `${leftPos}px`, top: `${topPos}px`, transition: "none",
        });
      }
    }

    createPreloader() {
      if (!this.canvasContainer) { console.warn("Canvas container not found. Skipping preloader."); return; }
      setTimeout(() => {
        this.canvasContainer.style.display = "block";
        this.canvasContainer.style.visibility = "visible";
        this.canvasContainer.style.opacity = "0";
        this.canvasContainer.style.transition = "opacity 0.2s ease-in";
        requestAnimationFrame(() => { this.canvasContainer.style.opacity = "1"; });
      }, 10);
      ...
      if (this.preloader && typeof this.preloader.hide === "function") {
        ...
        setTimeout(() => {
          if (this.preloader && typeof this.preloader.destroy === "function") {
            this.preloader.destroy(); this.preloader = null;
          }
          if (this.canvasContainer) { this.canvasContainer.style.backgroundColor = "transparent"; }
        }, 500);
    ...
    setExportMode(enabled) {
      ...
      if (enabled) { document.body.classList.add("export-mode"); }
      else { document.body.classList.remove("export-mode"); }
      this.resizeCanvas();
    }

    getCurrentScale() { return this.scaleValue || 1; }
    ...
    setFramerate(fps) {
      if (typeof fps !== "number" || fps <= 0 || fps > 240) {
        console.warn("Invalid framerate. Must be between 1 and 240 FPS."); return false;
      }
      this.currentFPS = fps; return true;
    }

    getFramerate() { return this.currentFPS; }

    setAdaptiveFramerate(enabled) { if (enabled) { this.currentFPS = 0; } }

    addGlobalEventListener(eventType, callback) {
      this.globalMouseListeners.push({ type: eventType, callback });
    }

    onMouseDown(callback) { this.addGlobalEventListener("mousedown", callback); return this; }
    onMouseUp(callback) { this.addGlobalEventListener("mouseup", callback); return this; }
    onMouseMove(callback) { this.addGlobalEventListener("mousemove", callback); return this; }

    onClick(child, callback) { if (child && child.onClick) { child.onClick(callback); } }
  ...
  // Expose globally so the class declaration below can extend it
  window.BoardBase = _BoardBase;
}
